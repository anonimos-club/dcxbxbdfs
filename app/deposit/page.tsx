...
export default function DepositPage() {
  ...
  useEffect(() => {
    ...
    const captureAndStoreTrackingParams = () => {
      ...
      // Only store if we have new parameters or if existing ones are expired
      const existingParams = localStorage.getItem("tracking_params")
      if (existingParams) {
        const parsed = JSON.parse(existingParams)
        if (parsed.expiry && parsed.expiry > Date.now()) {
          // Merge new parameters with existing ones, prioritizing new ones
          Object.keys(trackingParams).forEach((key) => {
            if (trackingParams[key] && key !== "timestamp" && key !== "expiry") {
              parsed[key] = trackingParams[key]
            }
          })
          parsed.timestamp = Date.now()
          localStorage.setItem("tracking_params", JSON.stringify(parsed))
          return
        }
      }

      // Store new parameters if any exist
      const hasParams = Object.keys(trackingParams).some(
        (key) => trackingParams[key] && key !== "timestamp" && key !== "expiry",
      )

      if (hasParams) {
        localStorage.setItem("tracking_params", JSON.stringify(trackingParams))
        console.log("[v0] Stored tracking parameters:", trackingParams)
      }

      // Append stored parameters to current URL for xTracky
      const storedParams = localStorage.getItem("tracking_params")
      if (storedParams) {
        const params = JSON.parse(storedParams)
        if (params.expiry > Date.now()) {
          const currentUrl = new URL(window.location.href)
          Object.keys(params).forEach((key) => {
            if (params[key] && key !== "timestamp" && key !== "expiry") {
      ...
      try {
        ...
        if (storedParams) {
          ...
          if (params.expiry > Date.now()) {
            ...
            console.log("[v0] Including tracking data in payment:", trackingData)
          }
        }
        ...
        const response = await fetch("/api/payments/create-transaction", {
          ...
          body: JSON.stringify({
            ...
            trackingData, // Include tracking parameters
          }),
        })
