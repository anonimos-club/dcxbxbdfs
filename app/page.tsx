...
export default function HomePage() {
  ...
  useEffect(() => {
    // Capture and store URL parameters for tracking
    const captureTrackingParams = () => {
      ...
      const hasTrackingParams = Object.values(trackingParams).some((value) => value !== null && value !== undefined)

      if (hasTrackingParams) {
        localStorage.setItem("tracking_params", JSON.stringify(trackingParams))
        console.log("[v0] Tracking parameters captured and stored:", trackingParams)
      }

      // Always try to restore and append tracking params to current URL
      const storedParams = localStorage.getItem("tracking_params")
      if (storedParams) {
        try {
          const params = JSON.parse(storedParams)
          // Check if params are not older than 30 days
          const thirtyDaysAgo = Date.now() - 30 * 24 * 60 * 60 * 1000

          if (params.timestamp && params.timestamp > thirtyDaysAgo) {
            // Append stored params to current URL for xTracky to access
            ...
            // Update URL without page reload if params were added
            if (currentUrl.search !== window.location.search) {
              window.history.replaceState({}, "", currentUrl.toString())
            }
          } else {
            // Remove expired tracking params
